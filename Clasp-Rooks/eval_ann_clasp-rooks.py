...
data_train = pd.read_csv("./Clasp-Rooks/Daten/data_train.csv", sep=";").drop("instance", axis=1)
    ...
    for l2 in l2_weights:
        for num_hidden_layers in [1, 2, 4]:
            for size_hl in [1, 2, 4]:
                ...
                count += 1
                print(f"\nClasp Rooks L1:{l1} | L2:{l2} | #HL.:{num_hidden_layers} | Size HL.:{size_hl} | Arch.:{architec} | Dropout:{do} | {count}/144 | MSELoss")
                ...
                losses.to_csv("./Clasp-Rooks/Daten/HPTuning.csv", sep=";", header=False, mode="a", index=False)